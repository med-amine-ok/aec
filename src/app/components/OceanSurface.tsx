...
// Ocean-only surface rendered with alpha:true so the fixed SkyBackground
...
/* ---- Shader chunks (same as OceanBackground so #include works) ---- */
...
export default function OceanSurface() {
  ...
  useEffect(() => {
    ...
    // alpha:true so the fixed sky layer underneath shows through
    ...
    // ---------- Stars texture (needed for ocean reflection sampling) ----------
    const starsSeed = 87;
    const gridSize = 64;
    ...
    // ---------- Sky uniforms (for lighting/reflections) ----------
    ...
    function setOceanUniforms(material: THREE.ShaderMaterial) {
    ...
    surfaceGeo.setIndex([2, 3, 0, 3, 1, 0]);
    ...
    const surfaceMat = new THREE.ShaderMaterial({
      ...
      transparent: true,
      uniforms: { _Time: timeUniform, _NormalMap1: normalMap1, _NormalMap2: normalMap2 },
    });
    setOceanUniforms(surfaceMat);
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { isVisible = entry?.isIntersecting ?? true; },
      { threshold: 0.05 }
    );
